...
class WorkOnTime {
  ...
  defineJob (options = {}) {
    ...
    this.jobs[name] = {
      name, worker,
      onStart, onComplete, onFail, storeResult, persistTaskStatus,
  ...
  add (userTask) {
    ...
    userTask.storeResult = userTask.storeResult || job.storeResult
    userTask.persistTaskStatus = findTaskStatusPersistance(userTask, job)
    ...
    if (when.type === 'cron') {
      ...
      const task = {
        ...
        when: when.value,
        uuid: uuidv4(),
        storeResult: userTask.storeResult,
        persistTaskStatus: userTask.persistTaskStatus,
        created: curTime,
        isActive: true,
        status: {}
      }
      return Promise.resolve().then(() => {
        if (userTask.persistTaskStatus) {
  ...
  _processTask (task, job) {
    ...
    return Promise.resolve().then(() => {
      ...
      if (job.onStart) {
        job.onStart(updatedTask)
      } else if (wot.onStart) {
        wot.onStart(updatedTask)
      }
      return job.worker(updatedTask)
    }).then(result => {
      // console.log('[wot] Task completed')
      finalResult = result
      const update = {status: {completed: Date.now()}}
      if (job.storeResult && notEmpty(result))  {
        extend(true, task, update);
        update.result = result
        return wot.store.patchTask(task.uuid, task)
      } else {
        return Promise.resolve().then(() => {
          if (task.persistTaskStatus) {
    ...
    .catch(error => {
      let logType = 'error'
      if (error.code && /IMAP_CONNECTION_TIMEOUT/i.test(error.code))
        logType = 'info'
      logger[logType]('[wot] Error: ' + error.message, {stack: error.stack})
