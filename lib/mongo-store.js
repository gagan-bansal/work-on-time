...
class MongoStore {
  constructor (opts = {}) {
    ...
    this.uri = opts.uri || 'mongodb://127.0.0.1:27017/workOnTime'
    const uriObj = mongodbUri.parse(this.uri);
    if (!uriObj.database === 'undefined')
      throw new Error('MongoDB wrong uri paramter: ' + this.uri)
    this.dbName = uriObj.database || opts.db || 'workOnTime'
  ...
  getCronTasks () {
    return this.collection.find({isCron: true}).toArray()
  }

  getActiveCronTasks (tillWhen, limit) {
    let query = { $and: [
      {isCron: true, isActive: true},
      {$nor: [
  ...
  // TODO name can not be tasks so can we change the array name
  getTasks (query = {}, opts = {}) {
