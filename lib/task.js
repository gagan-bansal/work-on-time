...
class Task extends EventEmitter {
  constructor (taskOptions = {}, worker, store, nowThreshold = 1000 ) {
    ...
    const {isCron, when} = getWhenType(taskOptions.when, nowThreshold);
    this.when = when;
    this.isCron = isCron;
    this.storeResult = taskOptions.hasOwnProperty('storeResult') ?
      taskOptions.storeResult :  worker && worker.hasOwnProperty('storeResult') ?
      worker.storeResult : false;
    this.persistTaskStatus = taskOptions.hasOwnProperty('persistTaskStatus') ?
      taskOptions.persistTaskStatus : worker && worker.hasOwnProperty('persistTaskStatus') ?
      worker.persistTaskStatus : true;
    this.persistCronStatus = taskOptions.hasOwnProperty('persistCronStatus') ?
      taskOptions.persistCronStatus : worker && worker.hasOwnProperty('persistCronStatus') ?
      worker.persistCronStatus : true;
...
function getWhenType (when, nowThreshold) {
  if (!when) {
    ...
    // its Date object
    if (when.valueOf() < Date.now() + nowThreshold) return false;
    return {isCron: true, when: when}
  } else if (Number.isInteger(when)) {
    // its epoch value
    if (when < Date.now() + nowThreshold) return false;
    return { isCron: true , when: new Date(when)}
  } else if (typeof when === 'string') {
    let val = filterInt(when);
    if (Number.isInteger(val)) {
      // string of numbers
      if (val < Date.now() + nowThreshold) return false;
