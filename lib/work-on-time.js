...
class WorkOnTime {
  constructor (options = {}) {  // required
    ...
    this._jobsInQueue = {}
    this.crons = {}
    this.nowThreshold = options.nowThreshold || 100
    this._started = false
  ...
  addTask (userTask) {  // required
    ...
    const task = new Task(userTask, worker, this.store);
    //return this.store.addTask(userTask, worker).then(task => {
  ...
  _loadCronTasks () {  // required
    ...
    .then(taskList => {
      ...
      return Promise.mapSeries(taskList, taskData => {
        ...
        if (!worker)
          return Promise.reject(new Error('Job not defined: ' + taskData.worker));
