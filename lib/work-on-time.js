...
class WorkOnTime extends EventEmitter {
  ...
  restart () {  // required
    const wot = this;
    if (this._started) {
      return this.stop().then(() => wot.start());
    } else {
      return this.start();
    }
  }
  ...
  addTask (userTask) {  // required
    ...
    let task;
    try {
      task = new Task(userTask, worker, this.store, this.nowThreshold);
    } catch (error) {
  ...
  _createCronTask (task) {  // required
    ...
    this._cronTasks[task.uuid] = task
    try {
  ...
  _processTask (task) {  // required
    ...
    return task.save().then(() => {
      ...
      task.emit('start', task.toPlainObject());
  ...
  _loadCronTasks () {  // required
    ...
    .then(taskList => {
      // console.log('cron tasks to be loaded: '+ taskList.length)
      // console.log('cron tasks list: '+ taskList.length, {taskList})
  ...
  getWorkers (options) {
    const list = Object.values(this.workers).map(worker => worker.toPlainObject());
    return list;
  }
  ...
  stopTask (uuid, reason) {
    if (this.crons[uuid]) {
      return this._stopCronTask(uuid, reason)
  ...
  _stopScheduledTask (uuid, reason) {
    ...
    if (task) {
      ...
      task.worker.emit('stop', task.toPlainObject());
      task.emit('stop', task.toPlainObject());
  ...
  deleteTask (uuid) {
    if (this.crons[uuid]) {
      return this._deleteCronTask(uuid, reason)
    } else if (this._jobsInQueue[uuid]) {
      return this._deleteScheduledTask(uuid, reason)
    } else {
      const error = new Error('No task: ' + uuid)
      error.statusCode = 404;
      return Promise.reject(error);
  ...
  async _deleteCronTask (uuid) {
    const task = this._cronTasks[uuid];
    this._stopCronTask(uuid);
    task.isActive = false;
    task.status.deleted = Date.now();
    try {
      await task.save();
    } catch (error) {
      return Promise.reject(error);
    }
    return Promise.resolve(task.toPlainObject);
  }
