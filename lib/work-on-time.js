...
class WorkOnTime {
  constructor (opts = {}) {  // required
    ...
    // this.onNotRunnable = opts.onNotRunnable
    this.setIntervalId
    this._jobsInQueue = {}
    this.crons = {}
  ...
  addWorker (worker) {
    ...
    if (this.jobs[worker.name])
      return new Error('Worker already added: ' + worker.name);
    this.jobs[worker.name] = worker;
  }
  _defineJob (options = {}) {  // not required
    const {
  ...
  _createCronTask (task) {  // required
    ...
    try {
      ...
      job.emit('fail', error, task.toPlainObject());
    }
    ...
    return task.save().then(() => {
      ...
      .then(result => {
        ...
        return task.save(result).then(() => {
          ...
          job.emit('complete', result, task.toPlainObject())
      ...
      .catch(error => {
        ...
        delete wot._jobsInQueue[task.uuid]
        /*if (job.onFail) {
          job.onFail(error, task.toPlainObject())
          .catch(err => {
        ...
        return task.save()
          .catch(error => {
            // TODO how to intimate wot error to wot
  ...
  _createTaskFromOld (oldTaskData) {
    ...
    // 2) retsart the old one keep status.created same and refress all
    //    other statuses.
